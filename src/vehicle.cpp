...
void Vehicle::BeginLoading()
{
	...
	//  for Existing objects tunnels and bridges as stations
		...
		assert(IsTileType(this->tile, MP_STATION) || IsTileType(this->tile, MP_TUNNELBRIDGE) || this->type == VEH_SHIP);
	}
...
void Vehicle::ShowVisualEffect() const
{
	...
	if (this->type == VEH_TRAIN) {
		...
		if (HasBit(t->flags, VRF_REVERSING) ||
				...
				((HasStationTileRail(t->tile) || IsTileType(t->tile, MP_TUNNELBRIDGE)) && t->IsFrontEngine() && t->current_order.ShouldStopAtStation(t, GetStationIndex(t->tile), IsRailWaypoint(t->tile)) &&
				t->cur_speed >= max_speed) ||
